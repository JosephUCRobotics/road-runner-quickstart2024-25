...
public final class AutoBlueState extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        topShootController = new PIDController(0.001, 0.00001, 0.00001);
        mainShootController = new PIDController(0.006, 0.00005, 0.00005);
...
//        Pose2d initialPose = new Pose2d(50, 52, -Math.PI/180 * 36);
        Pose2d initialPose = new Pose2d(-65, 14, Math.PI*.5);
        ...
        TrajectoryActionBuilder driveToShoot = drive.actionBuilder(initialPose)
                .setTangent(0)
                .splineToConstantHeading(new Vector2d(-60, 14), 0)
                .turnTo(Math.PI*.66);
        TrajectoryActionBuilder driveToPickup = drive.actionBuilder(new Pose2d(-60, 14, Math.PI*.66))
                ...
                .turnTo(Math.PI*.66);
        TrajectoryActionBuilder driveToPickup2 = drive.actionBuilder(new Pose2d(-60, 14, Math.PI*.66))
                .setTangent(0)
                .splineToSplineHeading(new Pose2d(-40,12,Math.PI*.5),0)
                .splineToConstantHeading(new Vector2d(-36, 55), Math.PI*.5)
                .setTangent(Math.PI*-.5)
                .splineToSplineHeading(new Pose2d(-60, 14, Math.PI*.66), Math.PI*-.5);
        ...
        public Action updateTray() {
            return new Action() {
                ...
                public boolean run(@NonNull TelemetryPacket packet) {
                    ...
                    if (startShootTime.milliseconds() > 1500 && mainShootTarget != 0) {
                        lever.setPosition(1);
                    } else {
                        lever.setPosition(.8);
                    }
...
//                    }
        ...
        public Action Shoot() {
            return new Action() {
                ...
                @Override
                public boolean run(@NonNull TelemetryPacket packet) {
                    if (!shooting && startShootTime.milliseconds() > 2000) {
                        shootTime.reset();
                        trayController.spinLeft(.2);
                        trayController.spinLeft(.2);
                        trayController.spinLeft(.2);
                        shooting = true;
                    }
                    ...
                    if (shooting && shootTime.milliseconds() > 3000) {
                        front_l.setPower(0);
                        back_l.setPower(0);
                        front_r.setPower(0);
                        back_r.setPower(0);

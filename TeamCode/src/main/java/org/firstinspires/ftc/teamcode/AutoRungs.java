...
@Autonomous(name = "AutoRungQualifier", group = "A")
public final class AutoRungs extends LinearOpMode {
...
//        }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        TrajectoryActionBuilder pathToPickUpBlock1 = drive.actionBuilder(new Pose2d(-28, 50, Math.PI*.75))
                .turnTo(Math.PI*.5);

        TrajectoryActionBuilder PathToHuman = drive.actionBuilder(new Pose2d(-28, 50, Math.PI*.5))
                .turnTo(Math.PI*.1);

        TrajectoryActionBuilder PathToWate = drive.actionBuilder(new Pose2d(-28, 50, Math.PI*.1))
                .setTangent(Math.PI*.5)
                .splineToSplineHeading(new Pose2d(-26, 62, Math.PI*.25), Math.PI, null, new ProfileAccelConstraint(-15, 15));
        ...
        TrajectoryActionBuilder PathToPickUpHook1 = drive.actionBuilder(new Pose2d(-26, 62, Math.PI*.25))
                .setTangent(-Math.PI)
                .splineToConstantHeading(new Vector2d(-48, 62), -Math.PI);
        TrajectoryActionBuilder PathToRung = drive.actionBuilder(new Pose2d(-48, 62, Math.PI*.25))
                .setTangent(0)
...
////                                arm.OpenClaw(),
////                                pathToHook1,
//                                arm.ShoulderMoveTo(1340, 1500, 2000, 1700),
//                                driveToPickUpHook1,
//                                arm.CloseClaw(),
//                                driveToRung,
//                                new ParallelAction(

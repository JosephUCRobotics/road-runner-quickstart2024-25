...
//        Action firstTrajectoryActionChosen = firstPathToBuckets.build();
        ...
        while (opModeInInit()) {
            ...
            redBlock.setPositionWithRobotPos(drive.pose);
...
//            double angle = angle2-angle1;
            telemetry.addData("robotHeading", Math.toDegrees(drive.pose.heading.toDouble()));
...
//                    drive.setPose(new Pose2d(24, z-32, Math.PI*.75));
                    ...
                    drive.setPose(new Pose2d(drive.getPose().position.x + ((24+yerror)-drive.getPose().position.x)*.04, drive.getPose().position.y +((xerror-10)-drive.getPose().position.y)*.03, drive.getPose().heading.toDouble()));

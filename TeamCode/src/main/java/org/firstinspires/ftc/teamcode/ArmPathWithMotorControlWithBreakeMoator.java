...
@TeleOp(name = "Arm With Break", group = "B")
@Config
public class ArmPathWithMotorControlWithBreakeMoator extends LinearOpMode {
    ...
    public static double AccelerationE = 2000;
    public static double MaxVelocityE = 2000;
    ...
    public static double KpB = 0.002;
    public static double KiB = 0.000;
    public static double KdB = 0.000;

    public static double BreakFF = 0.2;
    public static double ShoulderTargetOffset = 15;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            Map<String, Double> shoulderPosAndVel = calcCurrentTargetPos(move);
            double shoulderTarget = shoulderPosAndVel.get("targetPos");
            double shoulderTargetVel = shoulderPosAndVel.get("targetVel");

//            double breakPow = (shoulderTargetVel/MaxVelocity) * (1-minBreakPow)*breakMultiplyer + minBreakPow;
            double breakPow = 0;
            ...
            telemetry.addData("shoulderTargetVel", shoulderPosAndVel.get("targetVel"));
            telemetry.addData("breakPow", breakPow);
...
//            shoulderBreak.setPower(BreakFF);
    ...
    public double shoulderTarget(double reference) {
        ...
        telemetry.addData("Shoulder Err", error);
...
//        shoulderBreak.setPower(breakTarget(reference+ShoulderTargetOffset, encoderPosition));
        ...
        telemetry.addData("Shoulder out Pow: ",out);
...
//            }
    ...
    public double breakTarget(double reference, double encoderPosition) {
        // calculate the error
        double error = reference - encoderPosition;
        ...
        double out = (KpB * error) + (KiB * integralSumS) + (KdB * derivative);
        ...
        telemetry.addData("I Pow: ", Math.max(Math.min(KiS * integralSumS, 1), -1));
        telemetry.addData("D Pow: ", Math.max(Math.min(KdS * derivative, 1), -1));
        telemetry.addData("Break out Pow: ",out);
